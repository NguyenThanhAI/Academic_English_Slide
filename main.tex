	...
	\begin{figure}
		...
		\resizebox{11.0cm}{!}{
			\begin{tikzpicture}
				...
				\node[rectangle, draw=red!60, minimum size = 20mm, color=red] (loss) at (15, -7.5) {$\begin{aligned}\mathcal{L}&=\ell \Big(a_1^{(L)}, a_2^{(L)}, \dots, a_{d^{(L)}}^{(L)} \Big) \\&= \ell \Big( \mathbf{a}^{(L)} \Big) \end{aligned}$};
