...
\begin{frame}[allowframebreaks]{Fundamentals}
	\begin{figure}
		\centering
		\resizebox{5.5cm}{!}{%
		\begin{tikzpicture}
	...
	\begin{itemize}
		\item A neural network unit is a primitive neural network that consists of only an input layer, and an output layer with only one output.
  		\item A neural network unit computes a non-linear weighted combination of its inputs. 
	\end{itemize}


	What is the name of “neural networks” derived from?

	\begin{figure}
		\centering
		\includegraphics[width=10cm]{natural_neuron.png}
		\caption{Similarities between biological neurons and artificial neurons}
	\end{figure}
	\framebreak
	...
	Lying at the heart of ANN, backpropagation provides a powerful method to calculate derivatives of the loss function with respect to the current weights of the ANN, helping find out the proper set of weights for a reliable model.
\end{frame}

\begin{frame}[allowframebreaks]{Notations}
	Chain rule:
	\begin{itemize}
		\item A function $f(x)=f(g(h(x)))$, derivative of $f$ with respect to $x$:
		\begin{equation*}
			\dfrac{d f}{d x} = \dfrac{d f}{d g} \dfrac{d g}{d h} \dfrac{d h}{d x}
		\end{equation*}
		\item A function $f(x) = f\Big(z_1(x), z_2(x), \dots, z_n(x)\Big)$, derivative of $f$ with respect to $x$:
		\begin{equation*}
			\dfrac{d f}{d x} = \dfrac{\partial f}{\partial z_1} \dfrac{d z_1}{dx} + \dfrac{\partial f}{\partial z_2} \dfrac{d z_2}{dx} + \dots + \dfrac{\partial f}{\partial z_2} \dfrac{d z_2}{dx}=\sum_{i=1}^n \dfrac{\partial f}{\partial z_i} \dfrac{d z_i}{dx}
		\end{equation*}
	\end{itemize}
	\framebreak
	\begin{itemize}
...
\begin{frame}[allowframebreaks]{Experiment}
	The experiment uses the dataset \href{https://github.com/zalandoresearch/fashion-mnist}{Fashion Mnist} which contains 70,000 grayscale images of 10 categories, showing individual articles of clothing at low resolution (28 x 28 pixels)
	...
	The labels are an array of integers, ranging from 0 to 9. These correspond to the class of clothing the images represent:
	...
	\begin{figure}
		...
		\caption{A sample of images with corresponding labels}
	\end{figure}
	...
	\begin{itemize}
    	...
    	\item The output layer contains 10 neurons (equals the number of classes)
	...
	\begin{figure}
		...
		\caption{Neural network architecture used in the experiment}
	\end{figure}
